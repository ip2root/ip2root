...
def check_docker():
    ...
    if 'docker' not in res_docker:
        ...
        exit
    res_group = subprocess.check_output('id -nG "$(whoami)" | grep -qw "docke" && echo 1 || echo 0 && id -nG "$(whoami)" | grep -qw "steels" && echo 1 || echo 0', shell=True, universal_newlines=True)

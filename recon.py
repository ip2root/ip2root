...
def nmap_scan(ip: str, port: int | None) -> list:
    ...
    for port in open_ports:
        ...
        res_port = {'port': port['port_num'], 'proto': port['protocol'], 'service': port['service_type'], 'product': port['product_name'], 'version': port['product_version'], 'extrainfo': safe_get(port, 'extrainfo')}
        to_del = []
        for key in res_port:
            if res_port[key] == '':
                to_del.append(key)
